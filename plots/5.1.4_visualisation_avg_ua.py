...
colors = ['#d3d3d3', '#a9a9a9', '#696969', '#4b4b4b']  # Light to dark grey
hatches = ['/', '\\', '|', '-']

# Add horizontal grid lines
axes[0].yaxis.grid(True, linestyle='--', linewidth=0.7, color='gray')

# Plot 1: Percentage of Correct Answers with greyscale colors and hatch patterns
bars = axes[0].bar(percentage_correct.index, percentage_correct.values, color=colors, edgecolor='black')

# Apply hatches to bars
for i, bar in enumerate(bars):
    bar.set_hatch(hatches[i])

axes[0].set_xlabel('Варіація', fontsize=12)
axes[0].set_ylabel('Відсоток правильних відповідей', fontsize=12)

# Custom labels for the x-axis
custom_labels = ["Оригінальна", "Художня", "Суперечлива", "Прихована"]
...
# Annotate bar plot 1 with percentage values, highlighting "Cont disguise" (3)
for i, v in enumerate(percentage_correct.values):
    if i == 3:  # Assuming the last bar is "Adversarial"
...
axes[1].bar(solved_distribution.index, solved_distribution.values, color='gray', edgecolor='black', width=0.5)

axes[1].set_xlabel('К-ть розв\'язаних варіацій задач', fontsize=12)
axes[1].set_ylabel('К-ть задач', fontsize=12)
